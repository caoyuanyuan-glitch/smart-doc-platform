...
def get_progress(review_id: int):
    cached = _review_progress.get(review_id)
    if cached and cached.get('status') not in {None, '', 'unknown'}:
        return cached
    try:
        ...
        try:
            ...
            db.close()
        if not review:
            return {'status': 'unknown', 'step': '', 'progress': 0, 'message': ''}
        return {
            'status': review.status or 'unknown',
            'step': getattr(review, 'stage', '') or '',
            'progress': int(getattr(review, 'progress', 0) or 0),
            'message': getattr(review, 'message', None) or review.summary or '',
            'timestamp': (getattr(review, 'heartbeat_at', None) or datetime.utcnow()).isoformat(),
        }
    except Exception as exc:
        logging.getLogger(__name__).warning("读取审核进度失败 review_id=%s: %s", review_id, exc)
...
def set_progress(review_id: int, status: str, step: str, progress: int, message: str = '', allow_when_cancelled: bool = False):
    ...
    try:
        from app.database import SessionLocal
        from app.crud.review import persist_review_progress
        db = SessionLocal()
        try:
            persist_review_progress(db, review_id, status, step, progress, message)
        finally:
            db.close()
    except Exception as exc:
        logging.getLogger(__name__).warning("持久化审核进度失败 review_id=%s: %s", review_id, exc)
...
def _is_snippet_scope_issue(issue):
    ...
    if re.search(
        ...
        r"DOC-REV-|STRUCT-|SAFE-|XREF-|列宽|字号|图标大小|字体/版式",
        blob,
        re.IGNORECASE,
    ):
        return False
    if re.search(r"英文文档中文混入|ENG-CN-001", blob, re.IGNORECASE):
        return False
    if re.search(
        r"语法|拼写|术语|错别字|用词|标点|单位|空格|可读性|格式规范|商标|"
...
def _query_review_rows_for_user(
    ...
    if latest_only:
        latest_query = db.query(
            ReviewModel.document_id.label("document_id"),
            func.max(ReviewModel.id).label("latest_id"),
        ).join(DocumentModel, DocumentModel.id == ReviewModel.document_id).filter(DocumentModel.user_id == current_user.id)
        if document_id is not None:
            latest_query = latest_query.filter(ReviewModel.document_id == document_id)
        if normalized_status:
            latest_query = latest_query.filter(ReviewModel.status == normalized_status)
...
async def review_progress_stream(
    ...
    async def event_generator():
        last_hash = ""
        retry_count = 0
        max_retries = 900  # 15 分钟超时保护

        while retry_count < max_retries:
            progress = get_progress(review_id)
            status = progress.get("status", "unknown")

            # 计算内容哈希避免重复推送相同数据
            raw = json.dumps(progress, sort_keys=True, ensure_ascii=False)
            current_hash = hashlib.sha1(raw.encode()).hexdigest()
            ...
            # 如果状态是 unknown（可能还没开始），稍等更长时间
            sleep_time = 2.0 if status == "unknown" else 1.0
        ...
        if retry_count >= max_retries:
            progress = get_progress(review_id)
            yield f"data: {json.dumps({**progress, 'status': 'timeout', 'message': '审核进度推送超时'}, ensure_ascii=False)}\n\n"
...
def _review_pdf_visual_verify_enabled():
    return _review_env_bool('REVIEW_PDF_VISUAL_VERIFY', default=True)


def _review_pdf_visual_verify_limit():
    return max(0, _review_env_int('REVIEW_PDF_VISUAL_VERIFY_LIMIT', '6'))


def _review_pdf_visual_verify_scale():
    return max(1.0, min(3.0, _review_env_float('REVIEW_PDF_VISUAL_VERIFY_SCALE', '1.8')))
...
def _inject_comments_to_docx(docx_path, comments_data):
    ...
    try:
        from app.review_engine.safe_zip import safe_extract_zip
        extract_dir = os.path.join(temp_dir, 'extracted')
        safe_extract_zip(docx_path, extract_dir, allowed_prefixes=('word/', '[Content_Types].xml', '_rels/', 'docProps/'))
    ...
    if use_smart_chunking and create_smart_chunker is not None and len(content) > 1000:
        try:
            ...
            if smart_chunks:
                ...
                chunker_fallback_reason = "empty_smart_chunks"
        except Exception as e:
            print(f"[审核] 智能分块失败 ({e}), 回退到滑动窗口分块")
            all_chunks = _iter_ai_audit_chunks(content)
            chunks = _select_ai_audit_chunks(all_chunks, _review_ai_chunk_limit(len(content)))
            chunker_fallback_reason = str(e)
    else:
        all_chunks = _iter_ai_audit_chunks(content)
        chunks = _select_ai_audit_chunks(all_chunks, _review_ai_chunk_limit(len(content)))
        if create_smart_chunker is None:
            chunker_fallback_reason = "chunker_module_missing"
        elif not use_smart_chunking:
            chunker_fallback_reason = "smart_chunking_disabled"
        else:
            chunker_fallback_reason = "content_too_short"

    if not chunks:
        return [], {"enabled": False, "selected_chunk_count": 0, "total_chunk_count": 0, "chunks": [], "chunk_meta": [], "chunker_mode": chunker_mode, "fallback_reason": chunker_fallback_reason}
...
def _finalize_review_issues(issues, content, false_positive_signatures):
    ...
    ai_filter_diagnostics["filter_mode"] = "legacy" if use_legacy_post_filters else "pipeline"
    if use_legacy_post_filters:
        ...
        print("[审核] 兼容模式 REVIEW_USE_LEGACY_POST_FILTERS 已启用，结果可能与默认流水线不同")
...
def _load_false_positive_signatures_for_document(db: Session, document_id: int):
    signatures = set(list_false_positive_memory_signatures(db))
...
def _sync_false_positive_memory(db: Session, issue):
    status = str(getattr(issue, 'status', '') or '').lower()
    if status == 'false_positive':
        upsert_false_positive_memory_for_issue(db, issue, _issue_judgment_signatures(issue))
        return
    delete_false_positive_memory_for_issue(db, issue.id)
...
def _issue_page_number(issue, content):
    meta = _issue_meta(issue)
    for key in ('page_number', 'page', 'page_no'):
        ...
        try:
            ...
            continue
    return _page_number_from_position(content, _issue_value(issue, 'position', ''))
...
def _match_known_pdf_false_positive(issue):
    ...
    suggestion = str(_issue_value(issue, 'suggestion', '') or '').strip()
    category = str(_issue_value(issue, 'category', '') or '').strip()
    text_blob = ' '.join([original, context, description, suggestion, category])
    ...
    if re.search(r'missing\s+(?:icon|button|object)|缺少(?:按钮|图标|对象)', lowered, re.IGNORECASE):
        return 'missing_icon_text_layer_artifact'

    if re.search(r'重复文本层|双层文本层|text\s+layer\s+repeat|duplicate(?:d)?\s+text\s+layer|重复叠加', lowered, re.IGNORECASE):
        return 'duplicated_text_layer_artifact'
    if original and len(original) >= 4:
        compact_original = re.sub(r'\s+', '', original)
        if compact_original and compact_original * 2 in re.sub(r'\s+', '', context):
            return 'duplicated_text_layer_artifact'

    quote_chars = '“”‘’"\''
    if any(ch in text_blob for ch in quote_chars):
        if re.search(r'[“‘][^\n]{0,120}["\']', text_blob) or re.search(r'["\'][^\n]{0,120}[”’]', text_blob):
            return 'quote_mapping_artifact'
        if re.search(r'["”’][\.,;:](?:\s|$)', text_blob) and re.search(r'引号|quote|标点|punctuation|句号', lowered, re.IGNORECASE):
...
def _visual_provider_available(provider: str) -> bool:
    ...
    if name == "kimi":
        return bool(getattr(ai_client, "kimi_client", None))
    if name == "qwen":
        return bool(getattr(ai_client, "qwen_client", None) or getattr(ai_client, "dashscope_client", None))
    if name == "deepseek":
        return bool(getattr(ai_client, "deepseek_client", None))
    return False


def _verify_review_issue_visually(png_bytes, issue_payload, page_number, review_id):
    from app.review_engine.visual import map_visual_status, visual_provider_chain
    ...
    last = {
        ...
        "confidence": 0,
        "visual_status": "provider_unavailable",
        "provider": "",
        "attempts": 0,
    }
    for provider in visual_provider_chain():
        if not _visual_provider_available(provider):
            attempts.append({"provider": provider, "visual_status": "provider_unavailable"})
            continue
        if provider != "kimi":
            attempts.append({"provider": provider, "visual_status": "provider_unavailable", "reason": "vision_adapter_missing"})
            continue
        ...
        status = map_visual_status(result.get("decision"), result.get("reason"))
        result["visual_status"] = status
        result["provider"] = provider
        attempts.append({"provider": provider, "visual_status": status, "reason": str(result.get("reason") or "")[:180]})
    ...
    last["visual_status"] = "provider_unavailable"
    last["provider"] = ",".join(item["provider"] for item in attempts) if attempts else ""
    return last
...
def _apply_pdf_visual_verification(review_id: int, document, content: str, issues: list, suspicious_pages: dict | None = None):
    ...
    for issue in issues:
        if len(candidates) >= limit:
            break
        if not _should_visual_verify_issue(issue, getattr(document, 'file_type', '')):
            continue
        page_number = _issue_page_number(issue, content)
        if not page_number:
            continue
    ...
    visual_chain = visual_provider_chain()
    if not any(_visual_provider_available(name) for name in visual_chain):
    ...
    for issue, page_number in candidates:
        ...
        if decision == 'confirm':
            ...
            continue
        known_false_positive_reason = _match_known_pdf_false_positive(issue)
        if known_false_positive_reason:
            diagnostics['rejected_count'] += 1
            compact_result['decision'] = 'reject'
            compact_result['reason'] = known_false_positive_reason
            compact_result['is_extraction_artifact'] = True
            _normalize_issue_position_meta(issue, page_number=page_number, visual_verification=compact_result)
            diagnostics['items'].append({'page_number': page_number, **compact_result})
    ...
    filtered = [issue for issue in issues if id(issue) not in rejected_ids]
    print(f"[审核] PDF 渲染复核过滤: 过滤 {len(issues) - len(filtered)} 个, 剩余 {len(filtered)} 个")
...
def _get_document_upload_path(document):
    ...
    try:
        candidate = resolve_review_file_path(UPLOAD_DIR, getattr(document, "filename", "") or "")
...
def _load_cyy_human_review_basis_sections():
    ...
    sections.append({
        ...
        "priority": 5,
        "language": "both",
        "basis_type": "human_baseline_summary",
        "tags": ["cyy", "human-review", "summary"],
        "is_summary": True,
    })
    ...
    for item in annotations:
        ...
        if comment:
            ...
            sections.append({
                ...
                "tags": ["cyy", "human-review", category],
                "is_cyy_example": True,
            })
...
def _deduplicate_basis_sections(sections):
    seen = set()
    result = []
...
def _fetch_es_relevant_basis_sections(content, basis_sections, document_language, limit=4):
    try:
        service = get_audit_basis_search_service()
        from app.review_engine.basis_trace import build_basis_trace
        es_available = bool(service.is_available())
...
def _select_relevant_ai_review_basis_compat(content, basis_sections, document_language=None, max_sections=3, char_budget=3200):
    try:
        ...
        if "document_language" not in str(exc):
            raise
        return _select_relevant_ai_review_basis(content, basis_sections)


def _select_relevant_ai_review_basis(content, basis_sections, document_language=None, max_sections=3, char_budget=3200):
    if not basis_sections:
        return ""

    es_sections = _fetch_es_relevant_basis_sections(content, basis_sections, document_language, limit=max_sections + 1)
    if es_sections:
        selected = []
        total_length = 0
        preferred = []
        ...
        def _append_es_selected(section):
        ...
        for section in es_sections:
            label = str(section.get("label") or "")
            if section.get("is_checklist") or _is_checklist_basis_section(label):
                if checklist_section is None:
                    checklist_section = section
                continue
            if section.get("is_summary") or _is_summary_basis_section(label):
        ...
        if checklist_section is not None:
            ordered_es_sections.append(checklist_section)
        ordered_es_sections.extend(preferred)
        if summary_section is not None:
            ordered_es_sections.append(summary_section)

        for section in ordered_es_sections[:max_sections]:
            if not _append_es_selected(section):
                break
    ...
    for index, section in enumerate(basis_sections):
        ...
        if is_summary:
            if summary_entry is None or priority > summary_entry[2]:
                summary_entry = (section, text, priority)
        elif is_checklist:
            candidate = (relevance, overlap, priority, -index, section, text)
            if checklist_entry is None or candidate[:4] > checklist_entry[:4]:
                checklist_entry = candidate
        else:
...
_FILENAME_SPELLING_FIXES = {
    ...
    "Upgarde": "Upgrade",
    "Upgread": "Upgrade",
    "User's": "Users" if False else None,  # 仅用于提示"sesrise"等罕见情况
    "sesrise": "series",
    "serise": "series",
    "notifiction": "notification",
    "noitification": "notification",
    "updaet": "update",
    "verison": "version",
    "versoin": "version",
    "docuemnt": "document",
...
def _is_known_product_output_filename(stem: str) -> bool:
    ...
    return bool(re.fullmatch(
        r"[A-Z]{3,}(?:NAME|ID|NO|NUM|CODE)[A-Za-z][A-Za-z0-9_-]{3,}",
        normalized,
    ))


def _run_filename_audit(filename: str, content: str, document_language: str) -> list:
    """对文档文件名进行规则化审核。
    ...
    # 去掉扩展名用于分析
    stem = basename
    for ext in ['.docx', '.doc', '.pdf', '.md', '.txt', '.xlsx', '.xls', '.pptx', '.ppt']:
        if stem.lower().endswith(ext):
            stem = stem[:-len(ext)]
            break

    if _is_known_product_output_filename(stem):
        return []
    
    # ── 1. 拼写错误检测 ──
    for wrong, correct in _FILENAME_SPELLING_FIXES.items():
        if wrong in basename:
            issues.append({
                ...
                "suggestion": correct,
                "description": f"文件名中存在拼写错误：「{wrong}」应为「{correct}」",
                "audit_basis": "文件名拼写规范",
                "confidence": 95,
                "source": "rule",
                "position": "filename",
    ...
    # ── 2. 产品名一致性检查 ──
    # 从文件名提取可能的产品名模式（大写字母+数字+连字符组合，如 DNBSEQ-T7, MGISEQ-2000）
    # 只匹配含连字符的产品名，避免误匹配整个文件名
    ...
    # ── 3. 日期格式检查 ──
    # 检测 YYYYMMDD 格式（无分隔符）
    date_pattern_no_sep = re.findall(r'20\d{6}', stem)
    for date_str in date_pattern_no_sep:
        # 验证是否为合法日期
        try:
            ...
            issues.append({
                ...
                "context": f"文件名: {basename}",
                "suggestion": f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}",
                "description": f"文件名中的日期「{date_str}」建议使用 YYYY-MM-DD 格式（如 {date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}）",
                "audit_basis": "文件名日期格式规范",
                "confidence": 60,
                "source": "rule",
                "position": "filename",
            })
    ...
    if '  ' in basename:
        issues.append({
            ...
            "original_text": "  ",
            "context": f"文件名: {basename}",
            "suggestion": basename.replace('  ', ' '),
            "description": "文件名中存在连续空格，建议合并为单个空格",
            "audit_basis": "文件名格式规范",
            "confidence": 85,
            "source": "rule",
            "position": "filename",
    ...
    # ── 5. 版本号格式检查 ──
    # 检测 V 开头版本号
    version_patterns = re.findall(r'[Vv]\d+(?:\.\d+)*', stem)
    for ver in version_patterns:
        # 检查是否缺少点号分隔（如 V10 而非 V1.0）
        if re.match(r'^[Vv]\d{2,}$', ver):
            issues.append({
                "severity": "suggestion",
                "category": "文件名错误",
                "rule": "FILENAME-003",
                ...
                "context": f"文件名: {basename}",
                "suggestion": "",
                "description": f"文件名中的版本号「{ver}」可能缺少分隔符（如应为 V1.0 而非 V10）",
                "audit_basis": "版本号格式规范",
                "confidence": 60,
                "source": "rule",
                "position": "filename",
...
async def get_false_positive_memory_list(
    ...
    items, total = list_false_positive_memory(
        db,
...
@router.delete("/false-positive-memory/{memory_id}")
async def remove_false_positive_memory(
    memory_id: int,
    db: Session = Depends(get_db),
    _: UserOut = Depends(require_admin),
):
    entry = delete_false_positive_memory_entry(db, memory_id)
    if not entry:
        raise HTTPException(status_code=404, detail="False positive memory not found")
    return {"success": True}
    ...
    for rule in rules:
        try:
            ...
            for match in matches:
                ...
                issues.append({
                    "severity": getattr(rule, 'severity', None) or "general",
...
def _filter_low_quality_ai_issues(issues: list) -> list:
    """后处理质量过滤器：过滤AI产出的低价值/噪声问题。
    
    过滤规则：
    1. original_text 为单字符（包括单个标点、单字、单字母）
    2. original_text 与 expected 完全相同（空问题）
    3. original_text 为纯标点序列
    4. 规则名包含"标点"/"punctuation"且original少于3个有意义字符
    5. confidence < 50 的非交叉验证问题
    """
    if not issues:
        return []
    ...
    for issue in issues:
        ...
        confidence = issue.get("confidence", 0) or 0
        cross_validated = issue.get("cross_validated", False)
        
        # 规则1: 单字符问题（去除空格和标点后）
        meaningful_chars = re.sub(r"[\s.,;:!?()\[\]{}\"'`~@#$%^&*+=|\\<>/，。；：！？…—～（）【】《》""'']", "", original)
        if len(meaningful_chars) <= 1:
            dropped_count += 1
            continue
        
        # 规则2: original == expected（空问题）
        if original and expected and original == expected:
            dropped_count += 1
            continue
        ...
        # 规则4: 标点类规则 + 短文本
        punctuation_keywords = ["标点", "punctuation", "punct", "punctuation"]
        if any(kw in rule or kw in category or kw in issue_type for kw in punctuation_keywords):
    ...
    if dropped_count > 0:
        print(f"[质量过滤] 共过滤 {dropped_count} 个低质量问题，通过 {len(filtered)} 个")
    
    return filtered


def _merge_multi_provider_results(all_provider_issues: dict, provider_list: list) -> tuple:
    """合并多模型审核结果：去重 + 交叉验证标记。
    
    Args:
        all_provider_issues: {provider_name: [issue_dict, ...], ...}
        provider_list: ['qwen', 'deepseek', ...]
    
    Returns:
        (merged_issues, chunk_meta): 合并后的问题列表和元数据
    """
    merged = []
    
    for provider, issues in all_provider_issues.items():
        for issue in issues:
            # 标记来源 provider
            issue["_provider"] = provider
            merged.append(issue)
    ...
    # ── 跨模型去重 ──
    # 策略：基于 original_text 的 Jaccard 相似度去重
    # 保留第一个遇到的，并记录所有标记该问题的 provider
    deduped = []
    for issue in merged:
        ot = (issue.get("original_text") or "").strip().lower()
        if not ot:
        ...
        for existing in deduped:
            existing_ot = (existing.get("original_text") or "").strip().lower()
            if not existing_ot:
                continue
            # 完全相同 or 高度重叠（一个包含另一个且长度比 > 0.7）
            if ot == existing_ot:
                is_dup = True
            elif len(ot) > 10 and len(existing_ot) > 10:
                shorter = ot if len(ot) <= len(existing_ot) else existing_ot
                longer = existing_ot if len(ot) <= len(existing_ot) else ot
                if shorter in longer and len(shorter) / len(longer) > 0.7:
                    is_dup = True
            
            if is_dup:
                # 交叉验证：记录该问题也被当前 provider 发现
                existing_providers = set(existing.get("_providers", [existing.get("_provider")]))
                existing_providers.add(issue.get("_provider"))
        ...
        if not is_dup:
            issue["_providers"] = [issue.get("_provider")]
            deduped.append(issue)
    
    # 清理临时字段，标准化 providers
    for issue in deduped:
        providers = issue.pop("_providers", [issue.pop("_provider", "unknown")])
        issue["providers"] = json.dumps(providers, ensure_ascii=False)
        # 多模型交叉验证标记
        if len(providers) > 1:
    ...
    deduped = _filter_low_quality_ai_issues(deduped)
    filtered_out = pre_filter_count - len(deduped)
    if filtered_out > 0:
        print(f"[质量过滤] 过滤低质量问题 {filtered_out} 个，剩余 {len(deduped)} 个")
    
    multi_count = sum(1 for i in deduped if i.get("cross_validated"))
    single_count = len(deduped) - multi_count
    print(f"[多模型合并] 总问题={len(deduped)}, 交叉验证(多模型确认)={multi_count}, 单模型={single_count}")
    
    chunk_meta = [{
        "providers": provider_list,
        "raw_counts": {p: len(all_provider_issues.get(p, [])) for p in provider_list},
        "merged_count": len(deduped),
        "cross_validated_count": multi_count,
        "quality_filtered_count": filtered_out,
    }]
...
def _run_review_background(review_id: int, document_id: int, mode: str, provider: str = None, provider_list: list = None, visual_document_id: int = None, pairing_confirmed: bool = False):
    ...
    Args:
        provider: 指定单个 AI provider（向后兼容），如 'qwen'。None 时使用默认回退链。
        provider_list: 多个 provider 列表，如 ['qwen', 'deepseek']。
                      当指定多个时，AI深度审核将并发调用，结果去重合并。
    """
    from app.database import SessionLocal
    db = SessionLocal()

    # ── stage diagnostics tracking ───────────────────────────────
    _stage_start_times: dict[str, float] = {}
    _stage_input_counts: dict[str, int] = {}
    _stage_diagnostics: list[dict[str, Any]] = []

    def _begin_stage(name: str) -> None:
        _stage_start_times[name] = time.perf_counter()

    def _end_stage(name: str, output_count: int = 0, dropped_count: int = 0, errors: list[str] | None = None) -> None:
        duration_ms = int((time.perf_counter() - _stage_start_times.get(name, time.perf_counter())) * 1000)
        input_count = _stage_input_counts.get(name, 0)
        _stage_diagnostics.append({
            "stage": name,
            "input_count": input_count,
            "output_count": output_count if output_count else input_count,
    ...
    try:
        ...
        if engine_mode in ["rule", "hybrid"]:
            ...
            _begin_stage("rule_based_audit")
            _stage_input_counts["rule_based_audit"] = 0
            ...
            if document.file_type != 'xlsx' and not snippet_review:
                ...
                try:
                    ...
                    if filename_issues:
                        print(f"[审核] 文件名审核发现问题: {len(filename_issues)}个")
                        rule_issues.extend(filename_issues)
                except Exception as e:
                    print(f"[审核] 文件名审核执行失败: {e}")

            candidate_rule_issues = rule_issues + term_issues

            rule_stage_output = len(candidate_rule_issues)
        ...
        if document.file_type != 'xlsx' and engine_mode in ["ai", "hybrid"] and has_ai_client:
            ...
            # AI调用追踪
            try:
                from app.crud.audit_trace import sync_traces_from_usage_events
                usage_events = ai_client.get_usage_events(
                    request_label="review.audit_chunk",
                    review_id=review_id,
                    limit=100,
                )
                chunk_sizes = [m.get("chunk_size", 0) for m in ai_chunk_meta]
                issue_counts = [m.get("parsed_issue_count", 0) for m in ai_chunk_meta]
                if usage_events:
                    sync_traces_from_usage_events(db, review_id, usage_events, chunk_sizes, issue_counts)
                    print(f"[审核] AI调用追踪已持久化: {len(usage_events)} 条记录")
            except Exception as trace_err:
                print(f"[审核] AI调用追踪持久化失败(非致命): {trace_err}")
        ...
        summary = json.dumps({
            ...
            "filter_policy_version": REVIEW_FILTER_POLICY_VERSION,
...
def _normalize_providers(provider: str = None, providers: str = None) -> list:
    """将 provider/providers 参数规范化为单 provider 列表。
    
    Args:
        provider: 单个 provider 名称（向后兼容）
        providers: 兼容旧请求的逗号分隔 provider 名称
    
    Returns:
        list: 最多只返回一个 provider，如 ['qwen']
    """
    result = []
    if providers:
        result = [p.strip() for p in providers.split(",") if p.strip()]
    elif provider:
...
async def create_review_task(document_id: int, mode: str = "hybrid",
    ...
    from app.crud.review import reclaim_stale_running_reviews
    reclaim_stale_running_reviews(db, timeout_seconds=_review_env_int('REVIEW_TASK_HEARTBEAT_TIMEOUT', '900'))
...
def _dashboard_date_range(time_range='7d', start_date=None, end_date=None):
    ...
    return datetime.combine(start, dt_time.min), datetime.combine(end, dt_time.max)
...
def _gold_type_keyword_match(gold_type, issue):
    ...
    type_keywords = {
        ...
        "标点": ["标点", "punctuation", "punct"],
        "语法": ["语法", "grammar"],
        "合规": ["合规", "compliance", "监管"],
        "单位": ["单位", "unit"],
        "格式": ["格式", "format"],
        "操作": ["操作", "operation", "步骤"],
        "信息完整性": ["信息完整性", "信息", "completeness"],
        "表格": ["表格", "table"],
...
def _resolve_compare_row_level(field_def, main_value, reference_summary):
    ...
    if (
        field_name == "技术支持联系方式（电话/邮箱）"
        and not main_value
        and reference_values
        and all("邮箱:" in value and "电话:" not in value for value in reference_values)
    ):
        return "一致", "英文海外手册仅保留邮箱联系方式，参考文件口径可接受。"
    if not main_value and reference_summary["status"] != "missing":
...
async def compare_review_with_gold(
    ...
    print(f"[DEBUG gold-compare] review_id={review_id} gold_rows={len(gold_rows)}")
...
async def get_provider_status(_: UserOut = Depends(require_admin)):
    ...
    return {
        ...
        "provider_models": {
            ...
            "kimi": ai_client.kimi_model,
            "arkclaw": ai_client.arkclaw_model,
            "mcai": ai_client.mcai_model,
        ...
        "traces": [
            {
                ...
                "parsed_issue_count": t.parsed_issue_count,
...
async def get_stage_diagnostics(
    ...
    # 计算汇总统计
    total_duration_ms = sum(s.get("duration_ms", 0) for s in stage_diagnostics)
    total_dropped = sum(s.get("dropped_count", 0) for s in stage_diagnostics)
...
async def get_review_coverage(
    ...
    for t in traces:
        ...
        provider_stats[t.provider] = provider_stats.get(t.provider, 0) + 1

    # 7. 审核覆盖质量评分
    quality_score = _compute_coverage_quality_score(
        total, severity_dist, status_dist, traces, review
    )

    # 8. 规则覆盖度
    from app.crud.rule import get_rules as db_get_rules
    try:
        all_rules = db_get_rules(db, skip=0, limit=500)
        total_available_rules = len(all_rules)
    except Exception:
        total_available_rules = 0
    rules_triggered = len(rule_hit)
    rule_coverage_pct = round(rules_triggered / max(total_available_rules, 1) * 100, 1)
    ...
    return {
        ...
        "rule_hits": dict(sorted(rule_hit.items(), key=lambda x: x[1], reverse=True)[:20]),
        "rules_triggered": rules_triggered,
        "total_available_rules": total_available_rules,
        "rule_coverage_pct": rule_coverage_pct,
        "status_distribution": status_dist,
        "chapter_distribution": dict(sorted(chapter_dist.items(), key=lambda x: x[1], reverse=True)[:15]),
        "ai_stats": {
            "total_traces": len(traces),
            "total_tokens": total_tokens,
            "total_latency_ms": total_latency,
            "avg_latency_ms": round(total_latency / max(len(traces), 1)),
...
async def get_terminology_analysis(
    ...
    term_dicts = [
        {'id': t.id, 'non_standard': t.non_standard, 'standard': t.standard, 'category': t.category}
        for t in terms
    ]
    ...
    unmatched_terms = [
        {'id': t['id'], 'non_standard': t['non_standard'], 'standard': t['standard'], 'category': t['category']}
        for t in term_dicts if t['id'] not in matched_ids
    ]
...
async def reindex_document(
    ...
    try:
        ...
        engine.index_document(
            doc_id=review.document_id or review_id,
            content=content,
            title=review.document_name or '',
            language=review.document_language or 'unknown',
            review_id=review_id,
        )
...
async def run_batch_evaluation(
    ...
    接收评测集配置，运行多文档确定性规则审核并返回 EvalSuiteResult。

    支持两种模式：
    - dataset_id: 从默认评测集加载指定数据集
    - dataset_path: 从指定 JSON 文件加载
    """
    import asyncio
    import json as _json

    if not dataset_id and not dataset_path:
        raise HTTPException(status_code=400, detail="请提供 dataset_id 或 dataset_path")

    # Load config
    if dataset_path:
        config_path = Path(dataset_path)
        if not config_path.exists():
            raise HTTPException(status_code=404, detail=f"评测集配置文件不存在: {dataset_path}")
        config = _json.loads(config_path.read_text(encoding="utf-8"))
    else:
        default_path = Path(__file__).resolve().parents[3] / "eval" / "datasets" / "sample_baseline.json"
        if not default_path.exists():
            raise HTTPException(status_code=404, detail="默认评测集配置文件不存在")
        config = _json.loads(default_path.read_text(encoding="utf-8"))

    # Run synchronous eval in thread pool
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(
        None, _run_batch_eval_sync, config, dataset_id, ci_mode
    )
    return result
...
def _load_fixture_content(fixture_path: str) -> str:
    """Load fixture content from project root."""
    fp = Path(__file__).resolve().parents[3] / fixture_path
    if fp.exists():
        return fp.read_text(encoding="utf-8")
    return ""


def _compute_coverage_quality_score(total_issues, severity_dist, status_dist, traces, review):
    """计算审核覆盖质量评分 (0-100)"""
    score = 0
    fatals = severity_dist.get("fatal", 0)
    serious = severity_dist.get("serious", 0)
    confirmed = status_dist.get("confirmed", 0)
    false_pos = status_dist.get("false_positive", 0)

    # 检出问题 (30分)
    score += min(30, total_issues * 3)

    # 严重度价值 (20分)
    critical_ratio = (fatals + serious) / max(total_issues, 1)
    score += min(20, int(critical_ratio * 25))

    # 确认率 (20分)
    score += int(confirmed / max(total_issues, 1) * 20)
    score -= int(false_pos / max(total_issues, 1) * 15)

    # AI调用成功率 (15分)
    if traces:
        success = sum(1 for t in traces if t.status == "success")
        score += int(success / len(traces) * 15)
