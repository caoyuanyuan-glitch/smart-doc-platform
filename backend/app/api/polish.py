...
INLINE_SPACE_PATTERN = r'[\s\u00A0\u1680\u2000-\u200D\u202F\u205F\u3000\uFEFF]+'
LATIN_PHRASE_PATTERN = r'[A-Za-z0-9][A-Za-z0-9._%+-]*(?:[\s\u00A0\u1680\u2000-\u200D\u202F\u205F\u3000\uFEFF]+[A-Za-z0-9][A-Za-z0-9._%+-]*)*'
UNIT_TOKEN_PATTERN = r'(?:%|‰|mL|L|g|mg|kg|ng|pg|μg|ug|μL|uL|mM|μM|nM|mol|mmol|rpm|min|hr|h|s|ms|kb|bp|OD\d*|cm|mm|μm|um|nm|m)'
...
def _normalize_polish_spacing(text: str) -> str:
    """规整润色结果的中英数与单位空格。"""
    if not text:
        return text

    normalized = text
    normalized = re.sub(r'°\s*([CF])\b', r'°\1', normalized)
    normalized = re.sub(rf'([\u4e00-\u9fff]){INLINE_SPACE_PATTERN}({LATIN_PHRASE_PATTERN})', r'\1\2', normalized)
    normalized = re.sub(rf'({LATIN_PHRASE_PATTERN}){INLINE_SPACE_PATTERN}([\u4e00-\u9fff])', r'\1\2', normalized)
    normalized = re.sub(rf'([A-Za-z]+){INLINE_SPACE_PATTERN}(\d+(?:\.\d+)?)', r'\1\2', normalized)
    normalized = re.sub(rf'(\d+(?:\.\d+)?){INLINE_SPACE_PATTERN}([A-Za-z]+)', r'\1\2', normalized)
    normalized = re.sub(rf'(?<![A-Za-z])(\d+(?:\.\d+)?)(?:{INLINE_SPACE_PATTERN})?({UNIT_TOKEN_PATTERN})(?![A-Za-z])', r'\1 \2', normalized)
    return normalized


def _restore_temperature_degree_symbol(original: str, polished: str) -> str:
    if not polished:
        return polished
    ...
    if '℃' in source:
        restored = re.sub(rf'{signed_number_pattern}\s*(?:°C|℃|C)(?![A-Za-z])', r'\1 ℃', restored)
    elif '°C' in source:
        restored = re.sub(rf'{signed_number_pattern}\s*(?:℃|°C|C)(?![A-Za-z])', r'\1 °C', restored)

    if '℉' in source:
        restored = re.sub(rf'{signed_number_pattern}\s*(?:°F|℉|F)(?![A-Za-z])', r'\1 ℉', restored)
    elif '°F' in source:
        restored = re.sub(rf'{signed_number_pattern}\s*(?:℉|°F|F)(?![A-Za-z])', r'\1 °F', restored)
    return restored


def _change_type_priority(change_type: str) -> int:
    priorities = {
        'terminology': 5,
        'terminology_rule': 5,
    ...
    return priorities.get(change_type or '', 0)


def _dedupe_doc_changes(changes: list[dict]) -> list[dict]:
    deduped = {}
    ordered_keys = []
    for item in changes or []:
        ...
        key = (_normalize_compare_text(before), _normalize_compare_text(after))
        ...
        if existing is None:
            ...
            continue
        if _change_type_priority(change_type) > _change_type_priority(existing.get('type', '')):
            deduped[key] = candidate

    merged = []
    for key in ordered_keys:
        item = deduped[key]
        item_before = _normalize_compare_text(item.get('before', ''))
        item_after = _normalize_compare_text(item.get('after', ''))
        merged_existing = None
        for existing in merged:
            existing_before = _normalize_compare_text(existing.get('before', ''))
            existing_after = _normalize_compare_text(existing.get('after', ''))
            same_before = item_before and item_before == existing_before
            compatible_after = item_after == existing_after or item_after in existing_after or existing_after in item_after
            if same_before and compatible_after:
                merged_existing = existing
                break
        if merged_existing is None:
            merged.append(item)
            continue
        if len(item.get('after', '')) > len(merged_existing.get('after', '')):
            merged_existing['after'] = item.get('after', '')
        if _change_type_priority(item.get('type', '')) > _change_type_priority(merged_existing.get('type', '')):
            merged_existing['type'] = item.get('type', '')

    return merged


def _normalize_change_payload(changes: list) -> list:
    normalized_changes = []
    for change in changes or []:
        ...
        if isinstance(change, dict):
            ...
            if 'after' in normalized:
                normalized['after'] = _normalize_polish_spacing(_restore_temperature_degree_symbol(normalized.get('before', '') or normalized.get('original', ''), normalized.get('after', '')))
            if 'polished' in normalized:
                normalized['polished'] = _normalize_polish_spacing(_restore_temperature_degree_symbol(normalized.get('before', '') or normalized.get('original', ''), normalized.get('polished', '')))
...
def _extract_section_table_examples(section: str) -> list[str]:
    examples = []
    ...
    for line in lines:
        ...
        if header_cells is None:
            ...
            for idx, cell in enumerate(header_cells):
                ...
                if '示例' in cell or '推荐句式' in cell or 'example' in lower_cell:
                    example_index = idx
                    break
            continue
        if example_index < 0 or example_index >= len(cells):
            continue
        example = cells[example_index].strip().strip('"').strip("'").strip()
        if example:
...
def _has_equivalent_change(changes: list, before: str, after: str) -> bool:
    target_before = _normalize_compare_text(before)
    target_after = _normalize_compare_text(after)
    for item in changes or []:
        item_before = item.get('before', '') if isinstance(item, dict) else getattr(item, 'before', '')
        item_after = item.get('after', '') if isinstance(item, dict) else getattr(item, 'after', '')
        if _normalize_compare_text(item_before) == target_before and _normalize_compare_text(item_after) == target_after:
            return True
    return False
...
def _build_doc_change_details(original_text: str, polished_text: str, changes: list) -> list[dict]:
    ...
    return _dedupe_doc_changes(diff_rows)
...
def _filter_visible_doc_changes(changes: list) -> list[dict]:
    ...
    return _dedupe_doc_changes(visible)
...
def _extract_style_rules(guide_text: str) -> list[dict]:
    ...
    if guide_text and guide_text.strip():
        ...
        for section in sections:
            ...
            # 提取列表项作为具体规则
            items = re.findall(r'(?:^|\n)[-*]\s+(.+)', section)
            table_examples = _extract_section_table_examples(section)
            
            if not items and not table_examples:
                continue
            ...
            if table_examples:
                rules.append({
                    "type": "preferred_sentences",
                    "name": f"{header}-表格示例句式",
                    "sentences": table_examples,
                    "fix": "优先采用句式清单中的示例表达"
                })
        ...
        if rule["type"] == "forbidden_words":
            ...
            for sentence in rule.get("sentences", []):
                ...
                if normalized_result == normalized_sentence:
                    ...
                    break
                contains_match = (
                    normalized_result in normalized_sentence
                )
            ...
            if best_sentence and best_score >= 0.80 and result.strip() != best_sentence.strip():
                original = result
...
async def polish_text_endpoint(input_data: TextPolishInput, db: Session = Depends(get_db)):
    ...
    try:
        ...
        ai_polished = _normalize_polish_spacing(result.get("polished", input_data.text))
        ...
        polished = _restore_temperature_degree_symbol(input_data.text, polished)
        polished = _normalize_polish_spacing(polished)
        changes = []
        ...
        changes.extend(_normalize_change_payload(rule_changes))
        return {
            "original": result.get("original", input_data.text),
            "polished": polished,
            "changes": _normalize_change_payload(changes)
        }
    except Exception as e:
        print(f"[POLISH] 文本 AI 润色失败，回退规则润色: {e}")
...
async def polish_with_skill(
    ...
    try:
        ...
        if ai_result and ai_result.get("polished") and ai_result["polished"] != input_data.text:
            ai_polished = _normalize_polish_spacing(ai_result["polished"])
        ...
        for ac in ai_changes:
            if isinstance(ac, dict):
                changes.insert(0, PolishRuleMatch(
                    ...
                    after=_normalize_polish_spacing(ac.get("polished", ""))[:50],
                    type="ai"
                ))
...
def _polish_docx_with_comments(
    ...
    ai_lines: list = None,
    db_terminology: dict = None
) -> tuple[list[PolishRuleMatch], str]:
    ...
    final_paragraph_texts = [para.text or '' for para in doc.paragraphs]
    ...
    for i, (para_idx, para) in enumerate(non_empty_paras):
        ...
        if para_ai_change:
            para_changes.insert(0, para_ai_change)
        ...
        polished_text = _restore_temperature_degree_symbol(original_text, polished_text)
        polished_text = _normalize_polish_spacing(polished_text)
        if polished_text != original_text and not _has_equivalent_change(para_changes, original_text, polished_text):
            para_changes.append(PolishRuleMatch(
                rule_name="基础规范化",
                before=original_text[:100],
                after=polished_text[:100],
                type="format"
            ))
        final_paragraph_texts[para_idx] = polished_text
...
async def analyze_file_endpoint(
    ...
    try:
        ...
        if is_docx:
            ...
            changes, polished_text = _polish_docx_with_comments(
                temp_path, saved_file_path, skill_rules, db,
                sentence_guide, terminology, requirements,
                ai_lines=ai_lines,
                db_terminology=db_terms
            )
            polished_text = _restore_temperature_degree_symbol(content, polished_text)
            polished_text = _normalize_polish_spacing(polished_text)
            ...
            result_data = {
                ...
                "changes": _normalize_change_payload(display_changes),
...
async def polish_document(document_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "polished": _normalize_polish_spacing(_restore_temperature_degree_symbol(document.content or "", result.get("polished", document.content or ""))),
            "changes": _normalize_change_payload(changes)
        }
    except Exception:
...
def _polish_fallback(text: str, db_terminology: dict = None):
    ...
    polished = _restore_temperature_degree_symbol(text, polished)
    polished = _normalize_polish_spacing(polished)
    return {
        "original": text,
        "polished": polished,
        "changes": _normalize_change_payload(changes or [{"line": 1, "original": text[:80], "polished": polished[:80], "type": "format"}])
    }
