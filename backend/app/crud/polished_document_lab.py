...
from app.models.polished_document_lab import PolishedDocumentLab


def get_polished_documents(db: Session, skip: int = 0, limit: int = 100):
    return db.query(PolishedDocumentLab).order_by(PolishedDocumentLab.created_at.desc()).offset(skip).limit(limit).all()


def get_polished_document(db: Session, doc_id: int):
    return db.query(PolishedDocumentLab).filter(PolishedDocumentLab.id == doc_id).first()
