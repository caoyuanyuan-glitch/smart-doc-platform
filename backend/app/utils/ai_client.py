...
def _normalize_polish_spacing(text):
    if not text:
        return text
        ...
        for name, client, model in providers:
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    print(f"[AI] 命中模型: provider={name}, model={model}")
                    return response.choices[0].message.content
                except Exception as e:
                    error_str = str(e)
...
- 保持原句中中文、字母、数字之间的连接方式，不主动新增中英空格或数英空格
...
逐句改写，每句保留原文的具体名称和参数。"""
        ...
        try:
            ...
            if isinstance(parsed, dict) and "polished" in parsed:
                parsed["polished"] = _normalize_polish_spacing(parsed.get("polished", text))
                return parsed
        except:
            pass

        return {"original": text, "polished": _normalize_polish_spacing(result.strip())}
