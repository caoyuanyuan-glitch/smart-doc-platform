...
### 第 2.5 步：配置本地 AI 运行参数（图片描述生成必需）

首次在新电脑运行后端前，先复制 `backend/runtime.env.example` 为 `backend/runtime.env`，并填入真实的 Kimi Key。

```bash
# 在项目根目录执行
cp backend/runtime.env.example backend/runtime.env
```
...
# 首次运行前，先在 backend 目录创建 runtime.env
python init_data.py
