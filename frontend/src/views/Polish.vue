                ...
                <table class="doc-change-table">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in docResult.changeDetails" :key="`${index}-${item.before}-${item.after}`">
                      ...
                      <td class="col-before" v-html="renderDiffHtml(item.before, item.after, 'original', { structuredList: false })"></td>
                      <td class="col-after" v-html="renderDiffHtml(item.before, item.after, 'polished', { highlightWhitespace: true, structuredList: false })"></td>
    ...
    <div v-if="currentView === 'text'">
      ...
      <div class="content-row">
        <div class="content-left">
          <div ref="textInputPanelRef" class="panel" :style="textPanelStyle">
            <div class="panel-header">
        ...
        <div class="content-right">
          <div v-if="result" ref="textResultPanelRef" class="panel result-panel">
            <div class="panel-header">
              <span>润色结果</span>
              <div class="panel-actions">
            ...
            <div class="result-grid-vertical">
              <div ref="textResultCardRef" class="result-col-v">
      ...
      <div v-if="result" class="panel feedback-panel">
        ...
        <div class="feedback-bottom">
          <div class="form-item">
            <label class="form-label">写入目标（写入知识库对应文件中）</label>
...
const textResultPanelRef = ref(null)
...
let textResultResizeObserver = null
const textPanelHeight = ref(null)

function resetTextInputHeight() {
  textPanelHeight.value = null
}

function syncTextInputHeight() {
  const resultPanel = textResultPanelRef.value
  if (currentView.value !== 'text' || !result.value || !resultPanel) {
    resetTextInputHeight()
    return
  }
  const nextHeight = Math.max(Math.ceil(resultPanel.getBoundingClientRect().height), 0)
  if (nextHeight > 0) {
    textPanelHeight.value = nextHeight
  }
}

const textPanelStyle = computed(() => {
  if (!textPanelHeight.value || currentView.value !== 'text' || !result.value) {
    return {}
  }
  return {
    minHeight: `${textPanelHeight.value}px`
  }
})

function bindTextResultObserver() {
  if (typeof ResizeObserver === 'undefined') {
    return
  }
  if (!textResultResizeObserver) {
    textResultResizeObserver = new ResizeObserver(() => {
      syncTextInputHeight()
    })
  }
  textResultResizeObserver.disconnect()
  if (textResultPanelRef.value && currentView.value === 'text' && result.value) {
    textResultResizeObserver.observe(textResultPanelRef.value)
  }
}
...
const inlineWhitespaceRegex = /[\s\u00A0\u1680\u2000-\u200D\u202F\u205F\u3000\uFEFF]+/g
...
function stripInlineWhitespace(text) {
  return String(text || '').replace(inlineWhitespaceRegex, '')
}
...
function renderWhitespaceAlignedPolishedHtml(sourceText, targetText) {
  const sourceTokens = splitChars(sourceText)
  const targetTokens = splitChars(targetText)
  const output = []
  let i = 0
  let j = 0

  function appendToken(token, changeType = 'none') {
    if (!token) {
      return
    }
    const content = escapeHtml(token.text)
    const isWhitespaceOnly = /^\s+$/.test(token.text)
    if (changeType === 'space') {
      const visibleContent = content.replace(/ /g, '&nbsp;')
      output.push(`<span class="diff-highlight diff-highlight-space">${visibleContent}</span>`)
      return
    }
    if (changeType === 'adjacent') {
      output.push(`<span class="diff-highlight diff-highlight-adjacent">${content}</span>`)
      return
    }
    output.push(isWhitespaceOnly ? content.replace(/ /g, '&nbsp;') : content)
  }
  ...
  while (i < sourceTokens.length && j < targetTokens.length) {
    ...
    if (sourceToken.key === targetToken.key && !sourceToken.ignorable && !targetToken.ignorable) {
      appendToken(targetToken)
      i += 1
      j += 1
      continue
    }

    if (sourceToken.ignorable && !targetToken.ignorable && sourceTokens[i + 1] && sourceTokens[i + 1].key === targetToken.key) {
      appendToken(targetToken, 'adjacent')
      i += 2
      j += 1
      continue
    }

    if (targetToken.ignorable && !sourceToken.ignorable && targetTokens[j + 1] && targetTokens[j + 1].key === sourceToken.key) {
      appendToken(targetToken, 'space')
      appendToken(targetTokens[j + 1])
      i += 1
      j += 2
      continue
    }

    if (sourceToken.ignorable && targetToken.ignorable) {
      appendToken(targetToken)
      i += 1
      j += 1
      continue
  ...
  while (j < targetTokens.length) {
    const token = targetTokens[j]
    appendToken(token)
    j += 1
  }

  return `<div class="result-text-block">${output.join('')}</div>`
}
...
function renderSegmentsHtml(segments, options = {}) {
  const { highlightWhitespace = false } = options
  return mergeDiffSegments(segments)
    .map(segment => {
      const isWhitespaceOnly = /^\s+$/.test(segment.text)
      const content = isWhitespaceOnly && highlightWhitespace
        ? escapeHtml(segment.text).replace(/ /g, '&nbsp;').replace(/\n/g, '<br/>')
        : escapeHtml(segment.text)
      const highlight = segment.changed && (highlightWhitespace || !isWhitespaceOnly)
      if (!highlight) {
        return content
      }
      return isWhitespaceOnly
        ? `<span class="diff-highlight diff-highlight-space">${content}</span>`
        : `<span class="diff-highlight">${content}</span>`
    })
    .join('')
}

function renderDiffHtml(sourceText, targetText, mode, options = {}) {
  const { structuredList = true } = options
  if (mode === 'polished' && stripInlineWhitespace(sourceText) === stripInlineWhitespace(targetText) && String(sourceText || '') !== String(targetText || '')) {
    return renderWhitespaceAlignedPolishedHtml(sourceText, targetText)
  }

  const displayText = mode === 'original' ? sourceText : targetText
  const otherText = mode === 'original' ? targetText : sourceText
  const detectedList = structuredList ? detectLongParagraphList(displayText) : null

  if (detectedList) {
    const tagName = detectedList.type
    const itemsHtml = detectedList.items.map((item, idx) => {
      ...
      return `<li>${renderSegmentsHtml(segments, options).trim()}</li>`
    }).join('')
  ...
  const segments = mode === 'original' ? diff.left : diff.right
  return `<div class="result-text-block">${renderSegmentsHtml(segments, options)}</div>`
}
...
const highlightedPolishedHtml = computed(() => renderDiffHtml(result.value?.original, result.value?.polished, 'polished', { highlightWhitespace: true }))
const highlightedDocOriginalHtml = computed(() => renderDiffHtml(docResult.value?.original, docResult.value?.polished, 'original', { structuredList: false }))
const highlightedDocPolishedHtml = computed(() => renderDiffHtml(docResult.value?.original, docResult.value?.polished, 'polished', { highlightWhitespace: true, structuredList: false }))
...
function normalizeChangeCompareText(text) {
  return String(text || '').replace(/\s+/g, ' ').trim()
}

function compareChangeTypePriority(type) {
  const priorities = {
    terminology: 5,
    terminology_rule: 5,
    preferred_sentences: 4,
    style: 3,
    punctuation: 2,
    format: 1,
    ai: 0
...
function canMergeDocumentChange(target, candidate) {
  const targetBefore = normalizeChangeCompareText(target.before)
  const candidateBefore = normalizeChangeCompareText(candidate.before)
  const targetAfter = normalizeChangeCompareText(target.after)
  const candidateAfter = normalizeChangeCompareText(candidate.after)
  return targetBefore && targetBefore === candidateBefore && (
    targetAfter === candidateAfter ||
    targetAfter.includes(candidateAfter) ||
    candidateAfter.includes(targetAfter)
  )
}
...
function normalizeDocumentChanges(changes) {
  ...
  ;(changes || []).forEach((change, index) => {
    ...
    const normalizedBefore = normalizeChangeCompareText(before)
    const normalizedAfter = normalizeChangeCompareText(after)
    ...
    const dedupeKey = `${normalizedBefore}__${normalizedAfter}`
    const existingIndex = seen.get(dedupeKey)
    if (existingIndex !== undefined) {
      if (compareChangeTypePriority(change.type) > compareChangeTypePriority(deduped[existingIndex].type)) {
    ...
    const mergedTarget = deduped.find(item => canMergeDocumentChange(item, nextItem))
    if (mergedTarget) {
      if (String(after).length > String(mergedTarget.after).length) {
        mergedTarget.after = after
      }
      if (compareChangeTypePriority(change.type) > compareChangeTypePriority(mergedTarget.type)) {
        mergedTarget.type = change.type || ''
        mergedTarget.typeLabel = normalizeChangeType(change.type)
      }
      return
    }
...
watch([result, currentView], async () => {
  await nextTick()
  bindTextResultObserver()
  syncTextInputHeight()
}, { flush: 'post' })
...
.result-panel .result-col-v {
  display: block;
}

.result-panel .col-content-compact {
  min-height: 0;
  max-height: none;
}
...
.col-content-compact :deep(.diff-highlight-space) {
  display: inline-block;
  min-width: 0.6em;
  box-shadow: inset 0 0 0 1px #f87171;
}

.col-content-compact :deep(.diff-highlight-adjacent) {
  position: relative;
  border-radius: 3px;
}
...
.doc-change-table td :deep(.diff-highlight-space) {
  display: inline-block;
  min-width: 0.6em;
  box-shadow: inset 0 0 0 1px #f87171;
}

.doc-change-table td :deep(.diff-highlight-adjacent) {
  position: relative;
  border-radius: 3px;
}
