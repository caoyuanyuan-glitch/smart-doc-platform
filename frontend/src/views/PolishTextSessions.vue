<template>
  <div class="session-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">文本润色详情</h2>
        <div class="page-subtitle">查看文本润色反馈记录与评分详情</div>
      </div>
    ...
    <el-table :data="items" v-loading="loading" border stripe class="session-table" empty-text="暂无文本润色记录">
      <el-table-column prop="created_by" label="提交人" width="120" />
      <el-table-column prop="submitted_text" label="提交的文本" min-width="280" show-overflow-tooltip>
        <template #default="{ row }">
          <div class="multiline-cell">{{ row.submitted_text || '-' }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="match_rate" label="匹配率" width="100" align="center">
        <template #default="{ row }">{{ formatRate(row.match_rate) }}</template>
      </el-table-column>
      <el-table-column prop="accuracy" label="评分" width="100" align="center">
...
async function loadData() {
  ...
  try {
    ...
    const resp = await polishStatsAPI.getTextSessions(params)
...
.multiline-cell { display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.6; }
