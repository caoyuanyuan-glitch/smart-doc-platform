<template>
  <div class="competitor-container">
    <!-- 竞品列表页面 -->
    <div v-if="!selectedCompetitor" class="competitor-list">
    ...
    <!-- 竞品详情/对比页面 -->
    <div v-else class="competitor-detail">
      <div class="detail-header">
        <el-button @click="selectedCompetitor = null">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <div class="doc-section">
              ...
              <el-table :data="competitorDocs" v-loading="docsLoading" size="small" max-height="300">
          ...
          <el-col :span="12">
            <div class="doc-section">
              ...
              <el-table :data="ourDocs" v-loading="docsLoading" size="small" max-height="300">
                <el-table-column prop="file_name" label="文件名" min-width="150" />
                <el-table-column prop="version" label="版本" width="80" />
                <el-table-column prop="upload_date" label="上传时间" width="150">
                  <template #default="{ row }">{{ formatDate(row.upload_date) }}</template>
      ...
      <el-card class="compare-card">
        ...
        <h4 style="margin-top: 20px">对比历史</h4>
        <el-table :data="compareTasks" v-loading="tasksLoading" size="small">
          <el-table-column prop="id" label="ID" width="80" />
          ...
          <el-tab-pane label="内容差异">
            <el-table :data="resultData.content_diff" size="small" max-height="400">
              ...
              <el-table-column prop="type" label="类型" width="120">
                <template #default="{ row }">
                  <el-tag :type="getDiffType(row.type)" size="small">
                    {{ getDiffText(row.type) }}
                  </el-tag>
                </template>
              </el-table-column>
...
import { Plus, ArrowLeft, Upload } from '@element-plus/icons-vue'
...
async function uploadDocument() {
  ...
  try {
    await competitorAPI.uploadDocument(
...
async function loadCompareTasks() {
  ...
  try {
    const res = await competitorAPI.getCompareTasks(selectedCompetitor.value.id, {
      page: 1,
      page_size: 20
    })
...
async function startCompare() {
  if (!compareForm.competitorDocId || !compareForm.ourDocId) {
    ElMessage.warning('请选择竞品文档和我方文档')
    return
  }
  compareLoading.value = true
  try {
    const res = await competitorAPI.createCompare(
      selectedCompetitor.value.id,
      compareForm.competitorDocId,
      compareForm.ourDocId
    )
    ElMessage.success('对比任务已创建，正在处理')
    // 等待处理完成，轮询状态
...
async function viewCompareResult(row) {
  ...
  if (row.result_data) {
    try {
      ...
      resultData.structure_diff = data.structure_diff || []
      resultData.content_diff = data.content_diff || []
      resultData.chapter_similarity = data.chapter_similarity || []
    } catch (e) {
      resultData.structure_diff = []
      resultData.content_diff = []
      resultData.chapter_similarity = []
    }
  }
  // 解析建议
  if (row.suggestions) {
    try {
      suggestions.value = JSON.parse(row.suggestions)
    } catch (e) {
      suggestions.value = []
    }
  } else {
...
async function generateSuggestions(row) {
  try {
    ...
    const res = await competitorAPI.generateSuggestions(selectedCompetitor.value.id, row.id)
    suggestions.value = res.data.suggestions || []
    ElMessage.success('建议已生成')
    // 更新任务
    loadCompareTasks()
    if (currentTask.value && currentTask.value.id === row.id) {
      currentTask.value.suggestions = JSON.stringify(suggestions.value)
    }
  } catch (e) {
...
async function exportReport(row) {
  try {
    const res = await competitorAPI.exportReport(selectedCompetitor.value.id, row.id)
    const blob = new Blob([res.data], { type: 'text/html' })
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `竞品对比报告_${selectedCompetitor.value.name}_${new Date().toISOString().slice(0, 10)}.html`
    link.click()
    link.remove()
...
watch(() => route.params.id, (id) => {
  if (id) {
    competitorAPI.get(Number(id)).then(res => {
...
watch(showEditDialog, (val) => {
  if (val && selectedCompetitor.value) {
    editForm.name = selectedCompetitor.value.name
    editForm.brand = selectedCompetitor.value.brand || ''
    editForm.website = selectedCompetitor.value.website || ''
    editForm.description = selectedCompetitor.value.description || ''
    editForm.tags = selectedCompetitor.value.tags || ''
  }
})
</script>
...
.info-card, .docs-card, .compare-card {
  margin-bottom: 20px;
}
